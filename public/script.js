    ...
    .addEventListener("submit", async function (event) {
      ...
      try {
        ...
        const videoResponse = await fetch(
          `/api/youtube/video-details/${videoId}`
        );
        if (!videoResponse.ok) {
          throw new Error(
            `Error fetching video data: ${videoResponse.statusText}`
          );
        }
        const videoData = await videoResponse.json();

        const commentsResponse = await fetch(
          `/api/youtube/video-comments/${videoId}`
        );
        if (!commentsResponse.ok) {
          throw new Error(
            `Error fetching comments data: ${commentsResponse.statusText}`
          );
        }
        ...
        const commentsPagination = document.getElementById(
          "comments-pagination"
        );
        ...
        alert(
          "There was an error loading the video or comments. Please try again later."
        );
      }
    });
